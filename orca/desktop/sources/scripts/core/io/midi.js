...
function Midi (client) {
  ...
  this.trigger = function (item, down) {
    ...
    // Send to Web MIDI output if available
    if (this.outputDevice()) {
      this.outputDevice().send([c, n, v])
    }
    
    // Also send to RNBO Manager if available (for in-browser audio synthesis)
    if (typeof window !== 'undefined' && window.RNBOManager) {
      try {
        const status = c >> 4
        const note = n
        const velocity = v
        const midiChannel = c & 0x0F
        
        if (status === 9) { // Note on
          if (velocity > 0) {
            // playMidiNote is async, but we don't need to await here
            // as MIDI messages should be fire-and-forget
            window.RNBOManager.playMidiNote(note, velocity, midiChannel).catch(e => {
              console.warn('MIDI: RNBO playMidiNote error:', e.message)
            })
          } else {
            window.RNBOManager.stopMidiNote(note, midiChannel)
          }
        } else if (status === 8) { // Note off
          window.RNBOManager.stopMidiNote(note, midiChannel)
        }
      } catch (e) {
        // RNBO Manager not ready or error - continue without failing
        console.warn('MIDI: Could not send to RNBO Manager:', e.message)
      }
    }
    
    // If no Web MIDI output and no RNBO, log a warning
    if (!this.outputDevice() && (typeof window === 'undefined' || !window.RNBOManager)) {
